...
def sort(stack1, stack2, stack3):
    ...
    ## Check if only two values, then just sort straight ahead.
    ...
    ## Do first sort extracting from stack1 and putting into stack2
    anySwaps = False
    ...
    ## Initiate modified bubblesort
    stackToTransferTo = stack3
    stackToTakeFrom = stack2
    currentRun = 1
    while anySwaps == True:
        anySwaps = False
        value1 = stackToTakeFrom.pop()
        if stackToTakeFrom.empty():
            stack1.push(value1)
            continue
        value2 = stackToTakeFrom.pop()
        while stackToTakeFrom.empty() == False:
            if value1 > value2:
                anySwaps = True
                currentRun = 1
                stackToTransferTo.push(value2)
            else:
                currentRun += 1
                stackToTransferTo.push(value1)
                value1 = value2
            value2 = stackToTakeFrom.pop()
        ...
        for i in range(currentRun):
            stack1.push(stackToTransferTo.pop())
        
        if stackToTakeFrom == stack3:
            stackToTakeFrom = stack2
            stackToTransferTo = stack3
        else:
            stackToTakeFrom = stack3
            stackToTransferTo = stack2
